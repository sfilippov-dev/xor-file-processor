...
- **Корректная работа с путями на разных ОС.** Сравнение путей учитывает
  регистрозависимость файловой системы: Windows не различает регистр
  (`C:\Files` и `c:\files` - одно и то же), Linux и macOS различают.
